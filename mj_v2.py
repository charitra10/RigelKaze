...
AXONITE_HUNTER_ROLE_MIN_ROUND = INITIAL_RUSHER_MAX_ROUND + 1
AXONITE_CONVEYOR_VALIDATE_ROUNDS = 5
AXONITE_CONVEYOR_EMPTY_LIMIT = 20
AXONITE_CONVEYOR_CANDIDATE_LIMIT = 24
...
class Player:
    def __init__(self):
        ...
        self.seen_ally_titanium_harvester_conveyors: set[int] = set()
    ...
    def axonite_spawn_positions(self) -> list[Position]:
        ...
        for index in (1, 3, 2):
            if index < len(ordered):
                preferred.append(ordered[index])
        if self.in_bounds(self.core_pos) and self.core_pos not in preferred:
            preferred.append(self.core_pos)
        if ordered and ordered[0] not in preferred:
    ...
    def is_axonite_spawn_position(self, pos: Position) -> bool:
        if self.current_round < AXONITE_HUNTER_ROLE_MIN_ROUND:
            return False
        return any(pos == candidate for candidate in self.axonite_spawn_positions())
    ...
    def has_axonite_source_at(self, c: Controller, idx: int | None) -> bool:
        ...
        return (
            ...
            and c.get_team(building_id) == self.my_team
            and c.get_entity_type(building_id) == EntityType.HARVESTER
        )
    ...
    def finish_axonite_foundry(self) -> None:
    ...
    def run_axonite_hunter(self, c: Controller) -> None:
        if self.primary_direction is None and self.core_pos is not None:
            self.primary_direction = self.infer_primary_direction(c.get_position())
        self.waiting_for_titanium = False
        self.record_visible_titanium_harvester_conveyors(c)
    ...
    def acquire_axionite_target(self, c: Controller) -> bool:
        ...
        for tile in c.get_nearby_tiles():
            ...
            if idx in self.occupied_axionite or (TARGET_AXIONITE_ORE, idx) in self.bad_targets:
                ...
                continue
            if self.has_allied_harvester_on_axonite(c, tile):
                self.occupied_axionite.add(idx)
                if len(self.debug_blocked_ores) < 10:
                    self.debug_blocked_ores.append(idx)
                continue
    ...
    def has_allied_harvester_on_axonite(self, c: Controller, tile: Position) -> bool:
        if c.get_tile_env(tile) != Environment.ORE_AXIONITE:
            return False
        building_id = c.get_tile_building_id(tile)
        if building_id is None:
            return False
        return c.get_team(building_id) == self.my_team and c.get_entity_type(building_id) == EntityType.HARVESTER
    ...
    def configure_axonite_link_route_preferences(self, c: Controller) -> bool:
        self.maybe_switch_axonite_foundry_to_visible_harvester_site(c)
        if self.axionite_foundry_idx is None:
    ...
    def is_axonite_conveyor_candidate(self, idx: int) -> bool:
        ...
        return building_mine is not False

    def record_visible_titanium_harvester_conveyors(self, c: Controller) -> None:
        for building_id in c.get_nearby_buildings():
            if c.get_team(building_id) != self.my_team or c.get_entity_type(building_id) != EntityType.HARVESTER:
                continue
            harvester_pos = c.get_position(building_id)
            if c.get_tile_env(harvester_pos) != Environment.ORE_TITANIUM:
                continue
            for direction in CARDINAL_DIRECTIONS:
                conveyor_pos = harvester_pos.add(direction)
                if not self.in_bounds(conveyor_pos) or not c.is_in_vision(conveyor_pos):
                    continue
                conveyor_id = c.get_tile_building_id(conveyor_pos)
                if conveyor_id is None:
                    continue
                if c.get_team(conveyor_id) != self.my_team or c.get_entity_type(conveyor_id) != EntityType.CONVEYOR:
                    continue
                idx = self.pos_to_idx(conveyor_pos)
                self.seen_ally_conveyors.add(idx)
                self.seen_ally_titanium_conveyors.add(idx)
                self.seen_ally_titanium_harvester_conveyors.add(idx)

    def visible_titanium_harvester_foundry_conveyor(self, c: Controller) -> int | None:
        self.record_visible_titanium_harvester_conveyors(c)
        best: tuple[tuple[int, int, int], int] | None = None
        my_pos = c.get_position()
        for idx in self.seen_ally_titanium_harvester_conveyors:
            if not self.is_axonite_conveyor_candidate(idx):
                continue
    ...
    def maybe_switch_axonite_foundry_to_visible_harvester_site(self, c: Controller) -> bool:
        if self.ax_state != AX_STATE_ROUTE_TITANIUM:
            return False
        if self.axionite_foundry_idx in self.seen_ally_titanium_harvester_conveyors:
            return False
        candidate_idx = self.visible_titanium_harvester_foundry_conveyor(c)
        if candidate_idx is None or candidate_idx == self.axionite_foundry_idx:
            return False
        self.axionite_foundry_idx = candidate_idx
        self.reset_axonite_conveyor_validation()
        self.reset_target()
    ...
    def choose_axonite_foundry_conveyor(self, c: Controller) -> int | None:
        if self.axionite_harvester_idx is None:
            return None
        direct_idx = self.visible_titanium_harvester_foundry_conveyor(c)
        ...
        for idx in self.seen_ally_conveyors:
            ...
            score = (
                0 if idx in self.seen_ally_titanium_harvester_conveyors else 1,
        ...
        if not candidates:
            return None
        candidates.sort()
        for _, idx in candidates[:AXONITE_CONVEYOR_CANDIDATE_LIMIT]:
            goals, _ = self.axionite_link_route_goals(c, idx)
            if goals:
                return idx
        return None
        ...
        if building_id is not None:
            ...
            if building_type == EntityType.HARVESTER:
                if building_team == self.my_team:
                    ...
                    if self.axionite_harvester_idx != ore_idx:
                        self.axionite_harvester_idx = None
                    self.reset_target()
                    return False
    ...
    def wander_step(self, c: Controller) -> bool:
        ...
        current_pos = c.get_position()
        if self.role == ROLE_AXONITE_HUNTER:
            seed = self.wander_seed(c, current_pos)
            drift_options = (-2, -1, 0, 1, 2, -1, 1, 0)
            base_dir = rotate_direction(self.primary_direction, drift_options[seed & 7])
            turn = 1 if ((seed >> 3) & 1) == 0 else -1
            attempts = [0, turn, -turn, 2 * turn, -2 * turn, 3 * turn, -3 * turn, 4]
            if seed % 5 == 0:
                attempts = [turn, 0, 2 * turn, -turn, -2 * turn, 3 * turn, -3 * turn, 4]
            elif seed % 7 == 0:
                attempts = [2 * turn, turn, 0, -turn, -2 * turn, 3 * turn, -3 * turn, 4]
            for step in attempts:
                try_dir = rotate_direction(base_dir, step)
    ...
    def wander_seed(self, c: Controller, pos: Position) -> int:
        seed = (
            self.current_round * 1103515245
            + c.get_id() * 12345
            + pos.x * 73856093
            + pos.y * 19349663
            + (self.stuck_rounds + 1) * 83492791
        ) & 0x7fffffff
        return seed
