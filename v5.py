...
class Player:
    def __init__(self):
        ...
        self.occupied_titanium: set = set()   # set of Position — O(1) membership
        self.pos_markers_placed: set = set()   # set of Position — O(1) membership
        self.directions = [
            Direction.SOUTHEAST,
            Direction.SOUTHWEST,
            Direction.NORTHEAST,
        ...
        self.known_map: dict[int, str] = {}     # key = y*map_width+x
        ...
        self.observed_env: dict[int, Environment] = {}  # key = y*map_width+x
        # Precomputed (dx, dy, step_cost) for 8-directional and 4-directional movement
        # NORTH=(0,-1) SOUTH=(0,1) EAST=(1,0) WEST=(-1,0); diagonal cost ~14, cardinal 10
        self._dir_offsets_8 = [
    ...
    def route_builder_bot(self, ct: Controller) -> None:
        ...
        # Throttle symmetry detection — every 3 turns is enough and saves CPU
        if self.core_pos is not None and ct.get_current_round() % 3 == 0:
            ...
            if building_id is not None:
                if ct.get_entity_type(building_id) == EntityType.MARKER and tile not in self.pos_markers_placed and ct.get_team(building_id) == ally_team:
                    ...
                    if claimed_pos == self.titanium_currently_hunting_for:
                        self.titanium_currently_hunting_for = None
                        self.occupied_titanium.add(claimed_pos)
        ...
        if self.titanium_currently_hunting_for is not None:
            ...
            for tile in self.nearby_tiles:
                ...
                if (ct.get_tile_env(tile) == Environment.ORE_TITANIUM
                        ...
                        and ct.get_entity_type(building_id_scan) == EntityType.HARVESTER
...
# NEW — only checks tile when it's in vision, guards None, and returns after recursion
    ...
    def Place_marker(self, ct: Controller, i: Position) -> None:
        ...
        for d in self.directions:
            neighbour = i.add(d)
            if not self.is_in_bounds(neighbour):  # guard against out-of-bounds GameError
                continue
    ...
    def astar(self, start: Position, target: Position, max_nodes: int = 3000) -> list[Position]:
        ...
        INF = float('inf')
        size = W * H
        g = [INF] * size          # g_score flat array
        prev = [-1] * size        # came_from flat array (int idx, -1 = none)
        g[s] = 0

        open_set = [(0, 0, s)]    # (f, g_cost, idx)
        closed: set[int] = set()
        expansions = 0
        ...
        while open_set:
            ...
            if cur == t:
                # Reconstruct path by decoding int indices back to Position
                path = []
                c = cur
                while c != -1:
            ...
            if expansions >= max_nodes:
                return []  # budget exceeded — retry next turn
            ...
            for dx, dy, step_cost in offsets:
                ...
                if tile_state in ('building', 'enemy_core', 'builder_bot'):
                    if nxt != t:
                        continue
                
                # Add bridge penalty if it's a wall so bots prefer open terrain
                cost = step_cost
                if tile_state == 'wall':
                    cost += 90  # total 100 or 104
    ...
    def astar_cardinal(self, start: Position, target: Position, max_nodes: int = 2000) -> list[Position]:
        ...
        while open_set:
            ...
            for dx, dy, step_cost in offsets:
                ...
                cost = 10
                if tile_state == 'wall':
                    cost += 90  # bridge penalty

                tentative_g = g_cur + cost
                if tentative_g < g[nxt]:
                    g[nxt] = tentative_g
                    h = (abs(nx - tx) + abs(ny - ty)) * 10  # Manhattan for cardinal
                    prev[nxt] = cur
                    heapq.heappush(open_set, (tentative_g + h, tentative_g, nxt))
