        ...
        if self.titanium_currently_hunting_for is None:
            ...
            if self.titanium_currently_hunting_for is None:
                if ct.is_tile_passable(self.curr_builder_bot_pos.add(self.builder_bot_direction)):
                    ...
                    random_turn2=random.randint(0,1)
                    if(random_turn2==0):
...
# NEW — detect enemy road and attack it, then build on next turn
# Check if target tile has an enemy building blocking it
# NEW BLOCK — handle enemy road blocking the path (only when tile is visible)
            if target_build_tile in ct.get_nearby_tiles():
                try:
                    blocking_id = ct.get_tile_building_id(target_build_tile)
                    if blocking_id is not None and ct.get_team(blocking_id) == Team.B:
                        if ct.get_entity_type(blocking_id) == EntityType.ROAD:
                            if bot_pos.distance_squared(target_build_tile) <= 2:
                                if ct.get_action_cooldown() == 0 and ct.can_attack(target_build_tile):
                                    ct.attack(target_build_tile)
                            else:
