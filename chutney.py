...
class Player:
    def __init__(self):
        ...
        self.pending_ore_barrier_idx: int | None = None
    ...
    def should_spawn_hunter(self, c: Controller) -> bool:
        ...
        if titanium <= 1000 or c.get_unit_count() >= 50:
            return False
    ...
    def spawn_hunter(self, c: Controller) -> bool:
        if not self.should_spawn_hunter(c):
            return False
        spawn_pos = self.hunter_spawn_position()
        if spawn_pos is None or not c.can_spawn(spawn_pos):
            return False
        c.spawn_builder(spawn_pos)
        return True
    ...
    def run_axonite_hunter(self, c: Controller) -> None:
        ...
        if self.ax_state != AX_STATE_SEEK_AXIONITE and not self.has_allied_building_at(
            c, self.axionite_harvester_idx, EntityType.HARVESTER
        ):
            self.reset_axonite_mission()
        if self.ax_state in {AX_STATE_ROUTE_TITANIUM, AX_STATE_ROUTE_CORE} and not self.has_allied_building_at(
            c, self.axionite_foundry_idx, EntityType.FOUNDRY
        ):
        ...
        if self.ax_state == AX_STATE_ROUTE_TITANIUM:
            if not self.configure_foundry_route_preferences():
                self.ax_state = AX_STATE_BUILD_FOUNDRY
                return
        else:
        ...
        took_action = False
        if self.state == STATE_WANDER:
            if self.ax_state == AX_STATE_ROUTE_CORE:
                if self.axionite_foundry_idx is not None and self.plan_target(
                    c, TARGET_FOUNDRY_ROUTE, self.idx_to_pos(self.axionite_foundry_idx)
                ):
                    self.clear_move_path()
                else:
                    took_action = self.wander_step(c)
    ...
    def acquire_axionite_target(self, c: Controller) -> bool:
        ...
        for tile in c.get_nearby_tiles():
            ...
            if idx in self.occupied_axionite or (TARGET_AXIONITE_ORE, idx) in self.bad_targets:
                ...
                continue
            if self.is_axionite_occupied(c, tile):
                if len(self.debug_blocked_ores) < 10:
                    self.debug_blocked_ores.append(idx)
                continue
    ...
    def handle_axonite_foundry(self, c: Controller) -> bool:
        site_idx = self.choose_axonite_foundry_site(c)
        if site_idx is None:
        ...
        if c.can_build(EntityType.FOUNDRY, foundry_pos):
            c.build(EntityType.FOUNDRY, foundry_pos)
            self.ax_state = AX_STATE_ROUTE_TITANIUM
            return True
    ...
    def spawn_attackers_if_ready(self, c: Controller) -> bool:
        ...
        if not self.attack_wave_started or self.attackers_spawned >= ATTACKER_COUNT:
            return False
        for direction in self.attack_spawn_directions():
            spawn_pos = self.core_pos.add(direction)
            if c.can_spawn(spawn_pos):
                c.spawn_builder(spawn_pos)
    ...
    def initialize_attacker(self, c: Controller) -> None:
        self.reset_target()
        self.attack_phase = ATTACK_PHASE_CENTER if self.current_round <= INITIAL_RUSHER_MAX_ROUND else ATTACK_PHASE_RALLY
    ...
    def is_initial_rusher_position(self, pos: Position) -> bool:
        if self.current_round > INITIAL_RUSHER_MAX_ROUND or self.core_pos is None:
            return False
        directions = self.attack_spawn_directions()
        return bool(directions) and pos == self.core_pos.add(directions[0])
    ...
    def run_attacker(self, c: Controller) -> bool:
        ...
        if self.try_heal_attack_unit(c, urgent=took_damage):
            return True

        if took_damage:
            feeder = self.find_turret_supply_target(c, only_threatening=False)
            if feeder is not None:
                self.begin_attack_destroy(
                    feeder, ATTACK_TARGET_TURRET_FEEDER, ATTACK_PHASE_CUT_TURRET
                )

        if self.attack_phase == ATTACK_PHASE_CUT_TURRET:
            return self.handle_attack_destroy_target(c, ATTACK_PHASE_CORE)

        if self.attack_phase == ATTACK_PHASE_RALLY:
            center = self.get_map_center()
            if self.current_round - self.attack_spawn_round < ATTACK_RALLY_TURNS:
                if self.keep_attack_pair_together(c, center):
                    return True
                if pos_distance_sq(c.get_position(), center) > ATTACK_CORE_APPROACH_DIST_SQ:
                    return self.move_attacker_toward(c, center)
                return False
            self.attack_phase = ATTACK_PHASE_CENTER

        if self.attack_phase == ATTACK_PHASE_CENTER:
            center = self.get_map_center()
            if (
                self.attack_symmetry is None
                and len(self.attack_symmetry_candidates) > 1
                and pos_distance_sq(c.get_position(), center) > 4
            ):
                if self.keep_attack_pair_together(c, center):
                    return True
                return self.move_attacker_toward(c, center)
        ...
        if self.attack_phase == ATTACK_PHASE_CORE:
            enemy_core = self.current_enemy_core_guess()
            if enemy_core is None:
                return self.move_attacker_toward(c, self.get_map_center())
            if self.keep_attack_pair_together(c, enemy_core):
                return True
            feeder = self.find_enemy_core_feeder(c, enemy_core)
            if feeder is not None:
                self.begin_attack_destroy(
                    feeder, ATTACK_TARGET_CORE_FEEDER, ATTACK_PHASE_CUT_CORE_FEEDER
                )
                return self.handle_attack_destroy_target(c, ATTACK_PHASE_BUILD_GUNNER)
            return self.approach_enemy_core(c, enemy_core)

        if self.attack_phase == ATTACK_PHASE_CUT_CORE_FEEDER:
            return self.handle_attack_destroy_target(c, ATTACK_PHASE_BUILD_GUNNER)
        if self.attack_phase == ATTACK_PHASE_BUILD_GUNNER:
            return self.handle_attack_build_gunner(c)
        if self.attack_phase == ATTACK_PHASE_SUPPORT_GUNNER:
            return self.handle_attack_support_gunner(c)
    ...
    def update_attack_stuck(self, c: Controller, took_action: bool) -> None:
        ...
        self.attack_target_idx = None
        self.attack_target_kind = None
        if self.attack_phase in {
            ATTACK_PHASE_CUT_TURRET,
            ATTACK_PHASE_CUT_CORE_FEEDER,
            ATTACK_PHASE_BUILD_GUNNER,
        }:
            self.attack_phase = ATTACK_PHASE_CORE
        self.clear_move_path()
    ...
    def plan_target(self, c: Controller, target_kind: str, target_pos: Position) -> bool:
        ...
        self.pending_ore_barrier_idx = None
    ...
    def prepare_ore_target(self, c: Controller) -> bool:
        ...
        if c.can_build_harvester(ore_pos):
            ...
            if (
                ...
                self.pending_ore_barrier_idx = self.compute_ore_barrier_position(ore_pos)
            self.state = STATE_BUILD_ROUTE
            return True
    ...
    def handle_build_route(self, c: Controller) -> bool:
        ...
        if self.target_kind == TARGET_ORE and self.pending_ore_sentinels:
            ...
            if self.pending_ore_sentinels:
                return built
            return built
        if self.target_kind == TARGET_ORE and self.pending_ore_barrier_idx is not None:
            return self.place_ore_barrier(c)
    ...
    def compute_ore_barrier_position(self, ore_pos: Position) -> int | None:
        entry_dir = self.ore_entry_direction(ore_pos)
        if entry_dir is not None:
    ...
    def place_ore_barrier(self, c: Controller) -> bool:
        barrier_pos = self.idx_to_pos(self.pending_ore_barrier_idx)
        barrier_idx = self.pending_ore_barrier_idx
        current_idx = self.pos_to_idx(c.get_position())

        if pos_distance_sq(c.get_position(), barrier_pos) > 2:
            return self.move_toward_any(c, self.build_vantage_goals(barrier_pos))

        building_id = c.get_tile_building_id(barrier_pos)
        if building_id is not None:
            building_type = c.get_entity_type(building_id)
            building_team = c.get_team(building_id)
            ...
            if building_type == EntityType.MARKER:
                ...
                if c.can_destroy(barrier_pos):
                    c.destroy(barrier_pos)
                    return True
                return False
            elif building_type == EntityType.ROAD or building_type in ENEMY_REPLACEABLE_ROUTE_BUILDINGS:
                if current_idx == barrier_idx:
                    if c.can_fire(c.get_position()):
                        c.fire(c.get_position())
                        return True
                    return False
                return self.move_toward_any(c, {barrier_idx})
            self.pending_ore_barrier_idx = None
            return False

        if c.get_tile_env(barrier_pos) != Environment.EMPTY:
            self.pending_ore_barrier_idx = None
            return False
        if current_idx == barrier_idx:
            return self.move_off_current_tile(c, self.build_vantage_goals(barrier_pos))
        if c.can_build_barrier(barrier_pos):
            c.build_barrier(barrier_pos)
            self.pending_ore_barrier_idx = None
            return True
        self.wait_for_titanium_shortage(c, "get_barrier_cost")
        return False
    ...
    def complete_active_route(self) -> None:
        if self.role == ROLE_AXONITE_HUNTER:
            if self.ax_state == AX_STATE_ROUTE_TITANIUM:
                self.ax_state = AX_STATE_ROUTE_CORE
    ...
    def draw_debug(self, c: Controller) -> None:
        ...
        if self.role == ROLE_GUARDIAN:
            ...
            if self.hunt_target_pos is not None:
                c.draw_indicator_line(my_pos, self.hunt_target_pos, *COLOR_ENEMY)
                c.draw_indicator_dot(self.hunt_target_pos, *COLOR_ENEMY)
            for target_idx in self.hunt_debug_targets[:6]:
                c.draw_indicator_dot(self.idx_to_pos(target_idx), *COLOR_BRIDGE)
            if self.hunt_debug_goal_idx is not None:
                c.draw_indicator_line(my_pos, self.idx_to_pos(self.hunt_debug_goal_idx), *COLOR_ROUTE)
        elif self.role == ROLE_AXONITE_HUNTER:
            if self.axionite_harvester_idx is not None:
                harvester_pos = self.idx_to_pos(self.axionite_harvester_idx)
                c.draw_indicator_dot(harvester_pos, *COLOR_AXIONITE)
            else:
                harvester_pos = None
            if self.axionite_foundry_idx is not None:
                foundry_pos = self.idx_to_pos(self.axionite_foundry_idx)
                c.draw_indicator_dot(foundry_pos, *COLOR_AXIONITE)
                c.draw_indicator_line(my_pos, foundry_pos, *COLOR_AXIONITE)
            else:
                foundry_pos = None
            if harvester_pos is not None and foundry_pos is not None:
                c.draw_indicator_line(harvester_pos, foundry_pos, *COLOR_AXIONITE)
                for goal_idx in self.foundry_route_goals()[0]:
                    c.draw_indicator_dot(self.idx_to_pos(goal_idx), *COLOR_BRIDGE)
            if self.ax_state == AX_STATE_BUILD_FOUNDRY and foundry_pos is not None:
                c.draw_indicator_dot(foundry_pos, *COLOR_SENTINEL)
            if self.ax_state == AX_STATE_ROUTE_TITANIUM and self.target_idx is not None and foundry_pos is not None:
                c.draw_indicator_line(self.idx_to_pos(self.target_idx), foundry_pos, *COLOR_ROUTE)
            if self.ax_state == AX_STATE_ROUTE_CORE and foundry_pos is not None and self.core_pos is not None:
                c.draw_indicator_line(foundry_pos, self.core_pos, *COLOR_ROUTE)

        if self.target_idx is not None:
            target_pos = self.idx_to_pos(self.target_idx)
            if self.target_kind in {TARGET_ORE, TARGET_AXIONITE_ORE}:
                c.draw_indicator_line(my_pos, target_pos, *COLOR_ORE)
                c.draw_indicator_dot(target_pos, *COLOR_ORE)
            elif self.target_kind == TARGET_FOUNDRY_ROUTE:
                c.draw_indicator_line(my_pos, target_pos, *COLOR_AXIONITE)
                c.draw_indicator_dot(target_pos, *COLOR_AXIONITE)
            else:
        ...
        if self.pending_ore_barrier_idx is not None:
            c.draw_indicator_dot(self.idx_to_pos(self.pending_ore_barrier_idx), *COLOR_ROUTE)
